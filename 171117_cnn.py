...
class CNN:
    ...
    def modeling(self, optimizer):
        ...
        L1_identity = self.pooling(L1, 3, 2) # 8*8
        ...
        L10_1 = self.model(L9, 1, 1024, 256, 1, 'Layer_10_1', pad='VALID')
        L10_2 = self.model(L10_1, 3, 256, 256, 1, 'Layer_10_2')
        L10_3 = self.model(L10_2, 1, 256, 1024, 1, 'Layer_10_3', pad='VALID')
        L10 = tf.nn.relu(L9 + L10_3)
        L11_1 = self.model(L10, 1, 1024, 256, 1, 'Layer_11_1', pad='VALID')
        L11_2 = self.model(L11_1, 3, 256, 256, 1, 'Layer_11_2')
        L11_3 = self.model(L11_2, 1, 256, 1024, 1, 'Layer_11_3', pad='VALID')
        L11 = tf.nn.relu(L10 + L11_3)
        L12_1 = self.model(L11, 1, 1024, 256, 1, 'Layer_12_1', pad='VALID')
        L12_2 = self.model(L12_1, 3, 256, 256, 1, 'Layer_12_2')
        L12_3 = self.model(L12_2, 1, 256, 1024, 1, 'Layer_12_3', pad='VALID')
        L12 = tf.nn.relu(L11 + L12_3)
        L13_1 = self.model(L12, 1, 1024, 256, 1, 'Layer_13_1', pad='VALID')
        L13_2 = self.model(L13_1, 3, 256, 256, 1, 'Layer_13_2')
        L13_3 = self.model(L13_2, 1, 256, 1024, 1, 'Layer_13_3', pad='VALID')
        L13 = tf.nn.relu(L12 + L13_3)
        L14_1 = self.model(L13, 1, 1024, 256, 1, 'Layer_14_1', pad='VALID')
        L14_2 = self.model(L14_1, 3, 256, 256, 1, 'Layer_14_2')
        L14_3 = self.model(L14_2, 1, 256, 1024, 1, 'Layer_14_3', pad='VALID')
        L14 = tf.nn.relu(L13 + L14_3)
        L15_1 = self.model(L14, 1, 1024, 512, 2, 'Layer_15_1', pad='VALID')
        L15_2 = self.model(L15_1, 3, 512, 512, 1, 'Layer_15_2')
        L15_3 = self.model(L15_2, 1, 512, 2048, 1, 'Layer_15_3', pad='VALID')
        L15_identity = self.model(L14, 1, 1024, 2048, 2, 'Layer_15_identity', pad='VALID') # 1*1
        L15 = tf.nn.relu(L15_identity + L15_3)

        # model_ = self.pooling(L2, 8, 1, type='avg')
        model_ = tf.reshape(L15, [-1, 2048])
        W = tf.Variable(tf.random_normal([2048, 10], stddev=sqrt(4/2058)), name='fully-connected')
    ...
    def model(self, X, filter_size, in_feature, out_feature, stride, name, pad = 'SAME'):
        ...
        with tf.variable_scope(name):
            ...
            L = tf.nn.relu(L)
            mean, var = self.mean_var(L) if self.type == 'train' else self.mean_var(Xtr)
            L = tf.nn.batch_normalization(L, offset=beta, scale=gamma, mean=mean, variance=var, variance_epsilon=1e-3)
        return L
    # def conv_bn_relu(self, X, filter_size, in_feature, out_feature, stride, std, pad = 'SAME'):
    #     beta, gamma = self.beta, self.gamma
    #     W = tf.Variable(tf.random_normal([filter_size, filter_size, in_feature, out_feature], stddev=std))
    #     L = tf.nn.conv2d(X, W, strides=[1,stride,stride,1], padding=pad)
    #     mean, var = self.mean_var(L)
    #     L = tf.nn.batch_normalization(L, offset=beta, scale=gamma, mean=mean, variance=var, variance_epsilon=1e-5)
    #     L = tf.nn.relu(L)
    #     return L
    def mean_var(self, X):
        ...
        return mean, var
    def pooling(self, L, ksize, stride, type='max'):
        if type == 'max':
            L = tf.nn.max_pool(L, ksize=[1, ksize, ksize, 1], strides=[1, stride, stride, 1], padding='SAME')
        else:
            L = tf.nn.avg_pool(L, ksize=[1, ksize, ksize, 1], strides=[1, stride, stride, 1], padding='VALID')
        return L
    def save_model(self, sess, i):
        saver = tf.train.Saver()
        saver.save(sess, './save_model/cnn_model_{}.ckpt'.format(i))
...
if __name__ == '__main__':
    ...
    n = 0
    for op in ['adam', 'sgd']:
        for b in [0.0005*i for i in range(1, 10)]:
            for g in [0.0005*j for j in range(1, 10)]:
                with tf.Session() as sess:
                    ...
                    grid.to_csv('cnn_performance.csv')
                    print('epoch: %d\nop: %s\nbeta: %f\ngamma: %f\n' %(n, op, b, g))
                    n += 1